...
const v2 = new Array();     //  그냥 빈 배열
const v3 = new Array(1, "ABC", true);   //  초기화 배열 (어떤 객체든 가리지 않고 들어감) 
...
// JavaScript의 배열은 index 범위를 엄격하게 체크하지 않는다.
const arr = [];
...
console.log(fruits);
// 매개변수가 1개  :   해당 index부터 끝까지 추출 후 제거
// console.log(fruits.splice(2)); //   2번 index 부터 끝까지 ['Orange', 'Mango', 'Kiwi'] 를 빼내고 ['Apple', 'Banana'] 만 남김
// console.log(fruits);

// 매개변수가 2개 : 첫번째 매개변수 index ~ 두번째 매개변수 개수 만큼 추출 후 삭제
// console.log(fruits.splice(2,1));    //  ['Orange'] 만 빼냄
// console.log(fruits);

// 매개변수가 3개 이상 : 첫번째 매개변수 index ~ 두번째 매개변수 개수 만큼 추출 후 삭제하고 세번째 이후 매개변수를 요소로 추가
console.log(fruits.splice(2, 1, "Guava", "Grape")); //  ['Orange']를 삭제하고 그 자리에 ['Guava','Grape'] 추가
console.log(fruits);
